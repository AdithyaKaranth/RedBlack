/* The below code follows the algorithm in Thomas H Cormen. Read it foe more details*/ 
#include<stdio.h>
#include<stdlib.h>
struct NODE
{
...
void preorderDotDump (node root, FILE* outputFile,node nil)
{
	if (root !=nil) 
	{
		if(root->colour=='b')
			...
			fprintf (outputFile, "%d [label=%d,color=red,fontcolour=red];\n",root->key, root->key);
		if (root->left !=nil) 
